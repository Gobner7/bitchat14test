...
struct ChannelCrypto {
    struct Derived {
        ...
        let epochLenSec: UInt64 // epoch granularity for key separation
    }
    ...
    static func deriveKey(channelName: String, password: String, epochLenSec: UInt64 = 3600) -> Derived {
        ...
        return Derived(key32: keyData, channelID16: channelID, epochLenSec: epochLenSec)
    }
    ...
    static func seal(message: Data, channelKey: Derived, aad: Data?) throws -> ChannelEncryptedPacket {
        ...
        // Add epoch to AAD for context binding
        let epoch = currentEpoch(len: channelKey.epochLenSec)
        var epochBE = epoch.bigEndian
        let epochBytes = withUnsafeBytes(of: &epochBE) { Data($0) }
        let aadFull = (aad ?? Data()) + channelKey.channelID16 + epochBytes
        let sealed = try XChaCha20Poly1305Compat.seal(plaintext: message, key: channelKey.key32, nonce24: nonce, aad: aadFull)
        return ChannelEncryptedPacket(channelID: channelKey.channelID16, epoch: epoch, nonce24: nonce, ciphertext: sealed.ciphertext, tag: sealed.tag)
    }
    ...
    static func open(packet: ChannelEncryptedPacket, candidate: Derived, aad: Data?) throws -> Data? {
        ...
        // Accept current or adjacent epoch for slight clock skew
        let acceptableEpochs: [UInt64] = {
            let cur = currentEpoch(len: candidate.epochLenSec)
            return [cur, cur &+ 1, cur &- 1]
        }()
        guard acceptableEpochs.contains(packet.epoch) else { return nil }
        var epochBE = packet.epoch.bigEndian
        let epochBytes = withUnsafeBytes(of: &epochBE) { Data($0) }
        let aadFull = (aad ?? Data()) + candidate.channelID16 + epochBytes
    ...
    private static func currentEpoch(len: UInt64) -> UInt64 {
        let now = UInt64(Date().timeIntervalSince1970)
        return now / max(1, len)
    }
}
